...
valid_speakers = {"AiHoshino", "GawrGura", "DonaldTrump"}
speaker_file = "AiHoshinoV2.pth"  # ai hoshino by default
speaker_name = "Ai Hoshino"
speaker_pitch = 12
speaker_language = "japanese"
speaker_prompt = f"You are {speaker_name}. you will always responds as {speaker_name}. Please talk in {speaker_language}."

prev_speaker_name = speaker_name
# used for song covers as the global variables should be reset after the song is done
...
async def help(ctx):
    embed = discord.Embed(
        title="mibano", description="rvc discord bot by kenyoy", color=discord.Color.brand_green())

    embed.add_field(name="special thanks",
                    value=" Seshupengin-san, espided, and koitu da mod for tech support", inline=True)
...
async def say(ctx):
    try:
        ...
        speech_gen_file = speech_gen(
            ctx.message.content[5:])  # generated speech (GPT)
        rvc_audio_path = speech_convert(
            speech_gen_file)  # converted speech (RVC)
...
async def chat(ctx):  # response gen command
    ...
    # make sure no instance of line prompt splitter in message
    user_message.replace("§", "")

    # § indicates new line in message buffer
    message_buffer.append(f"user: {ctx.author.display_name}: {user_message}")
    # adds on the context to the message buffer
    ...
    if speaker_language != "english":  # translation if not english
        ...
        translation_eng = await translator(botResponse, "english")
        # turn to texting style
        informal_eng = await turn_informal(translation_eng)
...
async def advcover(ctx):
    ...
    try:
        ...
        if len(split_msg) != 7:
            ...
            reset_speaker()
            return
        # try the last one first so it doesnt start changing the vars
        audio_url = split_msg[6]
...
# dont need to pass in speaker_name and speaker_pitch cuz global
async def cover_gen(speaker_gain, reverb, start_time, audio_url, ctx):
    global speaker_file
    global speaker_pitch
    global isRVCGen
    ...
    # resets pitch but checks and changes speaker name
    isSuccessful = await parse_name(ctx, speaker_file, True)
    ...
    try:
        # for soundcloud, ytdlp doesnt remove file automatically for some reason
        os.remove(output_path + "youtube_audio.mp3")
    except Exception as e:
        print(colored(e, "red"))
    ydl_opts = {
        'quiet': True,
    ...
    trimmed_audio.write_audiofile(
        output_path + "trimmed_audio.mp3", codec='libmp3lame', bitrate="320k")
    ...
    # --tta --gpu 0
    subprocess.run(
        f'python "{remover_path}" --input "{trimmed_path}" --output_dir "{output_path}" --postprocess --tta --gpu 0', shell=True)
    ...
    if speaker_pitch % 12 == 0:
        ...
        # instead of pitching up by a lot, pitch down. save quality of audio...
        backing_pitch = -12 + speaker_pitch
    # same thing, ex) pitch voice down 8 semitones, then pitch up backing track 4 semitones -> same octave
    elif speaker_pitch < -6 and speaker_pitch > -12:
        backing_pitch = 12 + speaker_pitch
    else:
        backing_pitch = speaker_pitch
    print("speaker pitch: " + str(speaker_pitch) +
          " | backing pitch: " + str(backing_pitch))
    board2 = Pedalboard([PitchShift(semitones=float(backing_pitch)), Gain(
        gain_db=-3)])  # use pedalboard for doing audio fx
    ...
    # RVC webui is accessing this folder, must be from parent folder
    speech_convert_file = speech_convert(
        "../mibano" + output_path[1:] + "trimmed_audio_Vocals.wav")
    ...
    resample.export(output_path + "rvc_resampled.mp3",
                    format="mp3", bitrate="320k")

    # Apply Reveerb
    await ctx.send("(8/9) apply effects to voice")
    # Make a Pedalboard object, containing multiple audio plugins:
    # , , Convolution(impulse_response_filename = convolution_path, mix = float(reverb)/100)
    board = Pedalboard([Reverb(wet_level=reverb), Delay(
        delay_seconds=0.3, feedback=0.0, mix=reverb/20), Gain(gain_db=int(speaker_gain))])
    # Open an audio file for reading, just like a regular file:
    # Open an audio file to write to:
    with AudioFile(output_path + "rvc_resampled.mp3") as f:
        # Read one second of audio at a time, until the file is empty:
        with AudioFile(output_path + "rvc_reverb.mp3", 'w', f.samplerate, f.num_channels) as o:
            while f.tell() < f.frames:
    ...
    final_rvc_clip = AudioSegment.from_file(
        output_path + "rvc_reverb.mp3", format="mp3")
    final_backing_clip = AudioSegment.from_file(
        output_path + "backing_final.mp3", format="mp3")
    overlayed_audio = final_rvc_clip.overlay(final_backing_clip)
    overlayed_audio.export(
        output_path + "overlayed_audio.mp3", format="mp3", bitrate="320k")
...
async def convert_prompt(message_buffer):
    ...
    if (speaker_language != "english"):  # First message is system messages
        ...
        translated_prompt = speaker_prompt + \
            " as if you are talking in a voice call. Keep this to 1 sentence."
    ...
    for s in message_list:  # Loop over message_buffer and add each message to messages
        ...
        if split[1] == speaker_name:  # message of current bot speaker
            ...
            gpt_context.append(
                {"role": "assistant", "content": f'Assistant: "{split[1]}" says "{split[2]}"'})
        else:  # user message, other bots will be included as speaker
            gpt_context.append(
                {"role": "user", "content": f'user "{split[1]}" says "{split[2]}"'})
...
# {RVC_PATH}/assets/uvr5_weights/hoshino added_IVF337_Flat_nprobe_1_v2.index
    result = client_RVC.predict(
        # float (numeric value between 0 and 2333) in 'Select Speaker/Singer ID:' Slider component
        0,
        # str  in 'Enter the path of the audio file to be processed (default is the correct format example):' Textbox component
        str(speech_file_path),
        # float  in 'Transpose (integer, number of semitones, raise by an octave: 12, lower by an octave: -12):' Number component
        speaker_pitch,
        ...
        # str  in 'Select the pitch extraction algorithm ('pm': faster extraction but lower-quality speech; 'harvest': better bass but extremely slow; 'crepe': better quality but GPU intensive), 'rmvpe': best quality, and little GPU requirement' Radio component
        "rmvpe",
...
async def once_done(sink: discord.sinks, channel: discord.TextChannel, *args):
    ...
    # List down the files.
    files = [discord.File(audio.file, f"{user_id}.{sink.encoding}")
             for user_id, audio in sink.audio_data.items()]
...
# save file to the disk
    try:
        for user_id, audio in sink.audio_data.items():  # save the audio to the disk
            ...
            print(
                f'wav path {user_recording} {os.path.isfile(user_recording)}')
            # convertmp3
            ...
            # convert to mp3
            # convert to mp3
            user_recmp3 = f"./audio_export/voice_recording/uncut_recorded_audio_{user_id}.mp3"
            subprocess.run(["ffmpeg", "-i", user_recording, "-vn", "-ar", "44100",
                           "-y", "-loglevel", "error", "-ac", "2", "-b:a", "192k", user_recmp3])

            # generate response with fully appended user recording, Only do this if they stopped talking
            if is_mostly_silence(user_recording):
                ...
                message_buffer.append(
                    "user: " + user.display_name + ": " + user_response)
                ...
                message_buffer.append(
                    f"Assistant: {speaker_name}: " + str(botResponse))

                # speech
                speech_gen_file = str(speech_gen(botResponse))  # GPT
                rvc_audio_path = str(speech_convert(speech_gen_file))  # RVC
                rvc_audio = AudioSegment.from_wav(
                    rvc_audio_path)  # covert mp3 to wav
                rvc_audio.export(speech_convert_file,
                                 format="mp3", bitrate="320k")

                print("File: " + speech_convert_file)
                await channel.send(file=discord.File(speech_convert_file))

                sink.vc.play(discord.FFmpegPCMAudio(speech_convert_file), after=lambda e: print(
                    'Playing speech!', e))  # play the speech
                ...
                print(
                    colored("nah, there's enough noise here, seshpenguin-san!", "green"))
                # create file if not exist
                if not os.path.isfile(full_user_recording):
                    ...
                    try:
                        ...
                        audio_full.export(full_user_recording,
                                          format="mp3", bitrate="320k")
                    except Exception as e:
...
def is_mostly_silence(filename, silence_threshold=-70, nonsilent_chunk_threshold=1):
    # Load the audio file
    subprocess.run(["ffmpeg", "-i", f"{filename}", "-af", "apad=whole_dur=2",
                   # convert to mp3 and pad to 2s
                    f"{filename}.mp3", "-y", "-loglevel", "error"])
    ...
    nonsilent_chunks = detect_nonsilent(
        audio, min_silence_len=500, silence_thresh=silence_threshold)

    # Calculate total duration of nonsilent chunks
    total_nonsilent_duration = sum(
        [end - start for start, end in nonsilent_chunks])

    # Check if the total nonsilent duration is less than the threshold
    # if speaking is less than 30% of the audio, then it's mostly silence
    is_mostly_silent = total_nonsilent_duration < (0.3 * len(audio))
